...
//设置话术库
func SetWordsLibrary(c *gin.Context) {
	...
	if action == "ADD" { //添加话术
		...
		//fmt.Println(Chinese)
		//
		//DEC, _ := base64.StdEncoding.DecodeString(Chinese)
		//fmt.Println("chinese 值:"+string(DEC))
		...
		//判断是否重复添加
		err2 := mysql.DB.Where("chinese_md5=?", md5str1).First(&model.WordsArtLibrary{}).Error
		if err2 == nil { //找到了重复的数据
			util.JsonWrite(c, -101, nil, "不要重复添加")
			return
		}
		err := mysql.DB.Save(&model.WordsArtLibrary{Chinese: Chinese, India: India, English: English, ChineseMd5: md5str1, Created: time.Now().Unix()}).Error
		if err != nil {
	...
	if action == "GET" { //中文搜索 暂时只支持
		...
		words := make([]model.WordsArtLibrary, 0)
		...
		if username, isExist := c.GetPostForm("username"); isExist == true {
			for k, va := range words {
				//
				err := mysql.DB.Where("words_id=? and username=?", va.ID, username).First(&model.ThePhrase{}).Error
				if err == nil {
	...
	if action == "DEL" {
		...
		err := mysql.DB.Delete(&model.WordsArtLibrary{}, id).Error
		if err != nil {
	...
	if action == "collection" {
		...
		err1 := mysql.DB.Where("words_id=? and username=?", world.ID, username).First(&model.ThePhrase{}).Error
		if err1 == nil {
...
/***
  下载  话术
*/
func DownWordWords(c *gin.Context) {
...
上传话术
*/
func UploadWords(c *gin.Context) {
...
func SetThePhrase(c *gin.Context) {
	action := c.Query("action")
	...
	if action == "DEL" {
		...
		err := mysql.DB.Delete(&model.ThePhrase{}, id).Error
		if err != nil {
			util.JsonWrite(c, -101, nil, "删除失败")
			return
		}
