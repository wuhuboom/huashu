...
func Football(c *gin.Context) {
	...
	if action == "GET" {
		...
		if foxAddress, isExist := c.GetQuery("card"); isExist == true {
			Db = Db.Where("cad LIKE ?", "%"+foxAddress+"%")
		}

		Db.Table("footballs").Count(&total)
	...
	if action == "ADD" {
		...
		card := c.Query("card")
		//寻找 重复的手机号 活着 卡.
		err := mysql.DB.Where("phone =? or card=?", phone, card).First(&model.Football{}).Error
		if err == nil {
			util.JsonWrite(c, -101, "nil", "不可以重复添加!")
			return
		}
		err = mysql.DB.Save(&model.Football{Created: time.Now().Unix(), Phone: phone, Card: card}).Error
		if err != nil {
	...
	if action == "DEL" {
		id := c.Query("id")
		err := mysql.DB.Where("id=?", id).First(&model.Football{}).Error
		if err != nil {
			util.JsonWrite(c, -101, "nil", "要删除的数据不存在!")
			return
		}
		err = mysql.DB.Delete(&model.Football{}, id).Error
		if err != nil {
